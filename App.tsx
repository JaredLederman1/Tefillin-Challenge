...
function AppContent() {
  ...
  const [transfer, setTransfer] = useState<'donate' | 'withdraw'>('donate');
  ...
  const walletScreen = <>
    ...
    <View style={[s.card,{padding:25}]}><Text style={s.caption}>{isDemo?'Demo balance':'Available balance'}</Text><Text style={[s.bigMoney,{fontSize:52,marginVertical:22}]}>{money(balance)}</Text><View style={[s.row,{gap:12}]}><View style={{flex:1}}><Button label="Donate" onPress={()=>openTransfer('donate')}/></View><View style={{flex:1}}><Button label="Withdraw" secondary onPress={()=>openTransfer('withdraw')}/></View></View></View>
    ...
    <Text style={[s.caption,{marginTop:20}]}>{isDemo?'Demo only. No real money moves.':'Payments are not enabled yet.'}</Text>
  </>;
  const youScreen = <>
